...
function initDragAndDrop() {
  ...
  for (let cardsEl of $(".card")) {
    cardsEl.addEventListener("mousedown", function (e) {
      ...
      var gameSound = document.getElementById('clicksound');
        gameSound.play();
      isStart = 1;
      offsetX = e.offsetX;
      offsetY = e.offsetY;
  ...
  $(".full").addEventListener("click", function () {
    if (this.childNodes.length > 0) {
      ...
      for (let card of $(".view .card")) {
        ...
        var gameSound = document.getElementById('getback');
        gameSound.play();
      }
    }
  });
...
function allowDrop(parent, from) {
  totalScore += from == "stack" ? 5 : 10;
  updateScore();
...
function foundationCanStack(parent) {
  ...
  if (
    parseInt(parent.lastChild.getAttribute("data-type")) ===
    parseInt(selectedElements[0].getAttribute("data-type"))
  ) {
...
function stacksCanStack(parent) {
  ...
  if (
    parseInt(parent.lastChild.getAttribute("data-num")) - 1 ===
    parseInt(selectedElements[0].getAttribute("data-num"))
  ) {
    ...
    for (let foundation of $(".foundations")) {
      if (stack.childNodes.length > 0) {
        if (foundation.childNodes.length > 0) {
          if (
            stack.lastChild.getAttribute("data-type") ===
            foundation.lastChild.getAttribute("data-type")
          ) {
            if (
              parseInt(stack.lastChild.getAttribute("data-num")) ===
              parseInt(foundation.lastChild.getAttribute("data-num")) + 1
            ) {
